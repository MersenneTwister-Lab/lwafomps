...
test_that("mvnorm normal case", {
	...
	param.coval <- coval + d
	options(digits = 20)
	rs <- mvnorm(param.lower, param.upper, param.mean, param.coval)
	...
	expect_gt(rs$probability, 0.95)

	rs <- mvnorm(param.lower, param.upper, param.mean, param.coval, probability = 0.98)
	expect_equal(rs$status, "normal")
	expect_equal(rs$value, expected = 2.0599507948821, tolerance = 1e-06, scale = 1)
	expect_lt(rs$abseps, 1e-06)
	expect_lt(rs$releps, 1e-06)
	expect_gt(rs$probability, 0.98)
